...
mask_test = np.cumprod(~((fut_test[:, :, 0] == 0) * (fut_test[:, :, 1] == 0)), axis=0)
...
mean_err = np.sum((fut_test[:, :, :2] - pred_test[:, :, :2])*tiled_mask, axis=1, keepdims=True)/np.sum(tiled_mask, axis=1, keepdims=True)
# std_err_pred = np.sum(pred_test[:, :, 0, 2:4] * tiled_mask, axis=1) / np.sum(tiled_mask, axis=1)
std_err_pred = np.sum(pred_test[:, :, 2:4] * tiled_mask, axis=1) / np.sum(tiled_mask, axis=1)
...
for t in range(n_time_fut):
    var_xy_test2[t, :, :] = np.cov((fut_test[t, :, :2] - pred_test[t, :, :2]), rowvar=False,
...
print('FDE', latex_string(FDE[indices]))
print('rmse', latex_string(rmse_test[indices]))
print('miss rate', latex_string(miss_rate[indices]))
err_test_all = fut_test[:, :, :2] - pred_test[:, :, :2]
dist_error_all = np.sum(err_test_all*err_test_all, axis=2)*mask_test
...
var_line = plt.plot(var_xy_test[:, x_axis], var_xy_test[:, y_axis], color='green', label='std_xy(t)')
...
plt.legend([Line2D([0], [0], color='b', label='RMSE_xy(t)'),
            Line2D([0], [0], color='green', label='std_xy(t)'),
            # Line2D([0], [0], color='orange', label='batched_var_xy(t)'),
            Line2D([0], [0], color='red', label='sig_xy(t)'),
            ell_true, ell_pred],
           ['RMSE_xy(t)', "std_xy(t)", "sig_xy(t)", 'Global error covariance', 'Mean predicted error covariance'],
