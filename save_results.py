...
from utils.utils import Settings, get_multi_object_net, get_multi_object_test_set
...
if __name__ == '__main__':
    ...
    for j in range(len_test):
        hist, fut, mask_hist, mask_fut, _, _ = next(it_testDataloader)
        hist = hist.to(args.device)
        fut = fut.to(args.device)[:, :, 0, :]

        mask = torch.cumprod(1 - (fut[:, :, 0] == 0).float() * (fut[:, :, 1] == 0).float(), dim=0)

        fut_pred = net(hist)[-fut.shape[0]:, :, 0, :]
    ...
    # print('Test loss:', format(avg_loss / 100, '0.4f'))
