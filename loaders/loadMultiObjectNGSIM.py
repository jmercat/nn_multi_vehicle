...
class MultiObjectNGSIMDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #TODO: sort neighbors from most important to less important... (importance criterion to define)
        h_grid = (self.n_max_veh - 2) // 3
        ll_grid = 6 - h_grid//2
        for ind, i in enumerate(grid):
            if ll_grid <= ind % 13 <= ll_grid + h_grid:
    ...
    def getHistory(self, vehId, t, refVehId, dsId):
        if vehId == 0:
            return np.zeros([self.hist_len, 2])
        else:
    ...
    def collate_fn(self, samples):
        ...
        if self.n_max_veh>0:
            nbr_batch_size = self.n_max_veh - 1
        else:
            nbr_batch_size = 0
        ...
        for sampleId, (hist, fut, nbrs, nbrs_fut) in enumerate(samples):
            if nbrs is not None and len(nbrs)>nbr_batch_size:
                nbrs = nbrs[:nbr_batch_size]
                nbrs_fut = nbrs_fut[:nbr_batch_size]
            if hist is not None:
